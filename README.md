...
- **LAON VaultGuard** — 멀티 LLM 교차검증으로 Git 레포의 클라우드 프라이빗 키(AWS·Azure·GCP·KT·NCP)를 사전 탐지하는 보안 감사 도구. `git grep` 1차 필터 → LLM 문맥 분석 2단계, Ollama 오프라인 모드, VS Code 확장, Docker 지원. npm 패키지 + 웹 대시보드 + Slack·Telegram 알람.
