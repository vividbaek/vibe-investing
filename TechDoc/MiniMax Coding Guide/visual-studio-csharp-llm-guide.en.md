...
> A **sober, dry fact-check** of online material (written 2026-06), followed by recommendations for C# coding AI assistants in Visual Studio and **concrete ways to connect DeepSeek and other LLMs**.
> Companion doc: [MiniMax Coding Guide](minimax-coding-guide.en.md) (VS Code–centric)

- **As of**: 2026-06-04 · Verification: cross-checked against official marketplace, vendor docs, and public sources
- **Legend**: ✅ confirmed · ⚠️ corrected/caveat · ❓ could not confirm from public sources

---

## 0. First — Fact-Check Results (dry)

Verifiable claims from the source material, cross-checked against sources. **Items that could not be confirmed are honestly marked ❓.**

| Item | Source claim | Verification |
|---|---|---|
| GitHub Copilot agent mode | Automates navigation/refactoring/debugging in VS 2026 | ✅ True. Agent Mode is GA with MCP support. Supported on **VS 2022 17.14+ / VS 2026** |
| MCP + Roslyn semantic understanding | AI talks directly to Roslyn, F12-grade navigation | ⚠️ Mostly true but needs precision. Copilot offers language-aware symbol navigation via the `find_symbol` tool. "Direct Roslyn comms" holds **via MCP server extensions (e.g., MCP AI Server)** |
| MCP AI Server | ~20 Roslyn tools, FindSymbols | ✅ Real. `LadislavSopko/mcp-ai-server-visual-studio`, Roslyn-based 20 tools, for MCP clients |
| OpenCode AI Assistant | Roslyn symbol indexing, multi-model, MIT | ✅ Real on marketplace (`NatanaelNunez.opencode-ai-assistant-vs`). Multi-provider |
| Visual chatGPT Studio | Free, OpenAI-compatible API key | ✅ True. Free, needs OpenAI API key. Provides refactor/bug-find/test-gen commands |
| TONGYI Lingma | Alibaba, free | ⚠️ **Rebranded → "Qoder (formerly Lingma)"**. 3.5M+ cumulative downloads across channels. Free trial / limited-time free |
| MS C# ext unusable in Cursor | Blocked by license, netcoredbg alternative | ✅ True. MS restricts C#/C++/C# Dev Kit to MS-family editors. **Anysphere ships its own netcoredbg-based C# extension** (`@id:anysphere.csharp`) |
| ReSharper for VS Code | JetBrains, AI Assistant add-on | ✅ True. **Released 2026-03-05** (supports VS Code & Cursor). AI Assistant is paid (free for non-commercial/learning) |
| DeepSeek integration | Various AI models usable | ✅ DeepSeek is **OpenAI-compatible** (`https://api.deepseek.com`). Connectable from any extension that accepts an OpenAI-compatible base_url |
| A3sist | Local (Ollama)+MCP+RAG refactoring | ❓ **Could not confirm from public sources.** Verify existence/maintenance directly on marketplace/GitHub before adopting |
| L.AI | Local-focused, GitHub 298 | ❓ **Unconfirmed.** Verify directly likewise |
| Fitten Code / CodeAnalyzerAI | Autocomplete / real-time analysis | ❓ Only partially confirmed. Verify details/current maintenance directly |
| IntelliCode | Team-pattern-learning autocomplete | ✅ True (built into MS) |

> **Bottom line**: The big picture in the source (Copilot agent mode, MCP+Roslyn, Cursor's C# limitation, ReSharper for VS Code, the free extensions) is **largely accurate**. However, some minor extensions (A3sist, L.AI, etc.) **cannot be confirmed for existence/maintenance from public sources** and need direct verification before adoption.

---

## 1. Built-in AI in Visual Studio (check first)

- **GitHub Copilot** ✅ — Real-time completion + **agent mode** (VS 2022 17.14+ / VS 2026). Agent mode supports multi-file edits, iterative error fixing, tool calls, and **MCP server connections**. Custom agents can be defined via `.agent.md`.
- **IntelliCode** ✅ — Context-aware completion based on learned code patterns (no model setup needed).
...
| Extension | Core value | Multi-LLM connection | Price/License | Verified |
|---|---|---|---|---|
| **Visual chatGPT Studio** | Refactor/bug-find/test-gen commands, in-editor chat | OpenAI-compatible key + **Base URL override** → DeepSeek/MiniMax/local | Free (key needed) | ✅ |
| **OpenCode AI Assistant** | Large-solution focus, Roslyn symbol indexing | OpenAI/Anthropic/Ollama multi-provider | Free (MIT, key needed) | ✅ |
| **MCP AI Server** | Exposes 20 Roslyn tools via MCP (AI understands by meaning) | Model-agnostic (MCP client picks model) | Key/client separate | ✅ |
| **Qoder (formerly Tongyi Lingma)** | NL→code, multi-line generation | Alibaba's own models | Free trial / limited-time free | ⚠️ rebranded |
| **ReSharper + AI Assistant** | Best static analysis/refactoring + AI chat/completion | JetBrains AI Service (model choice) | Paid (free non-commercial) | ✅ |
| A3sist / L.AI | Claims local (Ollama) privacy focus | (verify) | (verify) | ❓ |
...
> ⚠️ **Disclaimer**: This document is a reference that verifies/organizes public information as of 2026-06-04. Extension install counts, prices, model names, and maintenance status change rapidly — reconfirm via official sources before adopting. For tools marked ❓ (A3sist, L.AI, etc.), verify existence/reliability directly before use. Manage API keys via environment variables / the IDE's secure storage and never commit them to a repository.
