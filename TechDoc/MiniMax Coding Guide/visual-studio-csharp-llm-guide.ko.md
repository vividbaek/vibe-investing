...
> 인터넷 자료(2026-06 작성)를 **드라이하게 사실 검증**한 뒤, Visual Studio에서 C# 코딩을 위한 AI 어시스턴트를 추천하고, **DeepSeek를 비롯한 다양한 LLM을 연결하는 실제 방법**을 정리한다.
> 자매 문서: [미니맥스 코딩 가이드](minimax-coding-guide.ko.md) (VS Code 중심)

- **기준일**: 2026-06-04 · 검증: 공식 마켓플레이스·벤더 문서·공개 출처 교차 확인
- **표기**: ✅ 확인됨 · ⚠️ 정정/주의 · ❓ 공개 출처로 확정 불가

---

## 0. 먼저 — 사실 검증 결과 (드라이)

원문 주장 중 검증 가능한 항목을 출처와 대조한 결과다. **확정하지 못한 항목은 솔직히 ❓로 표기**했다.

| 항목 | 원문 주장 | 검증 결과 |
|---|---|---|
| GitHub Copilot 에이전트 모드 | VS 2026에서 탐색·리팩토링·디버깅 자동화 | ✅ 사실. Agent Mode가 MCP 지원과 함께 GA. **VS 2022 17.14+ / VS 2026** 지원 |
| MCP + Roslyn 의미 이해 | AI가 Roslyn과 직접 통신, F12급 정확 탐색 | ⚠️ 대체로 사실이나 정밀화 필요. Copilot은 `find_symbol` 툴로 언어 인식 심볼 탐색 제공. "Roslyn 직접 통신"은 **MCP 서버 확장(예: MCP AI Server)을 통해** 성립 |
| MCP AI Server | Roslyn 20여 툴, FindSymbols | ✅ 실재. `LadislavSopko/mcp-ai-server-visual-studio`, Roslyn 기반 20개 툴, MCP 클라이언트용 |
| OpenCode AI Assistant | Roslyn 심볼 인덱싱, 멀티 모델, MIT | ✅ 마켓플레이스 실재(`NatanaelNunez.opencode-ai-assistant-vs`). 멀티 프로바이더 지원 |
| Visual chatGPT Studio | 무료, OpenAI 호환 API 키 | ✅ 사실. 무료, OpenAI API 키 필요. 리팩토링·버그탐지·테스트 생성 명령 제공 |
| TONGYI Lingma | 알리바바, 무료 | ⚠️ **리브랜딩됨 → "Qoder (구 Lingma)"**. 전 채널 누적 350만+ 다운로드. 무료 체험/한시 무료 형태 |
| Cursor에서 MS C# 확장 불가 | 라이선스로 차단, netcoredbg 대안 | ✅ 사실. MS가 C#/C++/C# Dev Kit를 MS 계열 에디터로 제한. **Anysphere가 netcoredbg 기반 자체 C# 확장** 배포(`@id:anysphere.csharp`) |
| ReSharper for VS Code | JetBrains, AI Assistant 추가 | ✅ 사실. **2026-03-05 출시**(VS Code·Cursor 지원). AI Assistant는 유료(비상업·학습용 무료) |
| DeepSeek 연동 | 다양한 AI 모델 사용 가능 | ✅ DeepSeek는 **OpenAI 호환**(`https://api.deepseek.com`). OpenAI 호환 base_url을 받는 확장이면 연결 가능 |
| A3sist | 로컬(Ollama)+MCP+RAG 리팩토링 | ❓ **공개 출처에서 확정 불가.** 도입 전 마켓플레이스·GitHub에서 실재·유지보수 직접 확인 권장 |
| L.AI | 로컬 특화, GitHub 298 | ❓ **확정 불가.** 동일하게 직접 확인 필요 |
| Fitten Code / CodeAnalyzerAI | 자동완성·실시간 분석 | ❓ 부분적으로만 확인. 세부 수치·현행 유지 여부는 직접 확인 권장 |
| IntelliCode | 팀 패턴 학습 자동완성 | ✅ 사실(MS 기본 제공) |

> **요지**: 원문의 큰 그림(Copilot 에이전트 모드, MCP+Roslyn, Cursor C# 제약, ReSharper VS Code, 무료 확장들)은 **대체로 정확**하다. 다만 일부 마이너 확장(A3sist·L.AI 등)은 **공개 출처로 실재·유지보수를 확정하기 어려워** 도입 전 직접 확인이 필요하다.

---

## 1. Visual Studio 기본 내장 AI (가장 먼저 검토)

- **GitHub Copilot** ✅ — 실시간 자동완성 + **에이전트 모드**(VS 2022 17.14+ / VS 2026). 에이전트 모드는 다중 파일 편집, 오류 반복 수정, 도구 호출, **MCP 서버 연결**을 지원한다. `.agent.md`로 커스텀 에이전트도 정의 가능.
- **IntelliCode** ✅ — 코드 패턴 학습 기반 문맥 자동완성(모델 추가 불필요).

> 별도 키·설정 없이 시작하려면 **Copilot + IntelliCode**가 출발점이다. 단, Copilot 기본 모델은 OpenAI/Anthropic이며, 임의의 외부 OpenAI 호환 엔드포인트(DeepSeek 등) 직결은 제한적이다(2장·3장의 외부 확장으로 우회).

---

## 2. C# 개발자용 AI 확장 추천 (Visual Studio)

> 설치 수·가격은 시점에 따라 변하므로 **마켓플레이스에서 현행 수치 재확인**을 전제로 읽을 것.

| 확장 | 핵심 가치 | 멀티 LLM 연결 | 가격/라이선스 | 검증 |
|---|---|---|---|---|
| **Visual chatGPT Studio** | 리팩토링·버그탐지·테스트 생성 명령, 에디터 내 챗 | OpenAI 호환 키 + **Base URL 오버라이드** → DeepSeek/MiniMax/로컬 | 무료(키 필요) | ✅ |
| **OpenCode AI Assistant** | 대규모 솔루션·Roslyn 심볼 인덱싱 | OpenAI·Anthropic·Ollama 등 멀티 프로바이더 | 무료(MIT, 키 필요) | ✅ |
| **MCP AI Server** | Roslyn 20개 툴을 MCP로 노출(AI가 의미 단위 이해) | 모델 비종속(MCP 클라이언트가 모델 선택) | 키/클라이언트 별도 | ✅ |
| **Qoder (구 Tongyi Lingma)** | 자연어→코드, 멀티라인 생성 | 알리바바 모델(자체) | 무료 체험/한시 무료 | ⚠️ 리브랜딩 |
| **ReSharper + AI Assistant** | 정적분석·리팩토링 최강 + AI 챗/완성 | JetBrains AI Service(모델 선택) | 유료(비상업 무료) | ✅ |
| A3sist / L.AI | 로컬(Ollama) 프라이버시 특화 주장 | (확인 필요) | (확인 필요) | ❓ |
...
- **로컬·프라이버시**: 1차로 **Ollama + OpenAI 호환 확장**(아래 3.3) 권장. A3sist/L.AI는 실재·유지보수 확인 후 채택

---

## 3. DeepSeek 및 다양한 LLM 연결 방법 (핵심)

원리는 하나다. **대부분의 상용 LLM은 OpenAI 호환(`/v1/chat/completions`) 엔드포인트를 제공**하므로, "Base URL + API Key + 모델명"만 바꾸면 같은 확장에서 여러 모델을 쓸 수 있다.
...
- 커스텀 OpenAI 호환을 고르고 **Base URL + Key + 모델명**을 DeepSeek/MiniMax/로컬로 지정
- 대규모 솔루션에서 **Roslyn 심볼 인덱싱**으로 프로젝트 전체 타입 인지 → 정확한 컨텍스트 제공
...
- **MCP AI Server**(Roslyn 20개 툴)를 설치하면, MCP 클라이언트(예: Copilot 에이전트 모드)가 `FindSymbols` 등 **컴파일러 수준 도구**를 호출해 단순 텍스트가 아닌 심볼 단위로 코드를 다룬다.
...
> ⚠️ **면책**: 본 문서는 2026-06-04 기준 공개 정보를 검증·정리한 참고자료다. 확장의 설치 수·가격·모델명·유지보수 상태는 빠르게 바뀌므로 도입 전 공식 출처로 재확인할 것. ❓로 표기한 도구(A3sist·L.AI 등)는 실재·신뢰성을 직접 확인한 뒤 사용하라. API 키는 환경변수/IDE 보안 저장소로 관리하고 저장소에 커밋하지 말 것.
